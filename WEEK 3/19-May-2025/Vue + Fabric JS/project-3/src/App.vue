...
export default {
  ...
  const circle = new fabric.Circle({
    ...
    top: 50,
    radius: 50,
    // fill will be replaced by a gradient in a moment
    fill: null,
    originX: 'center',
    originY: 'center'
  });

  // Build a radial gradient that matches the circle’s bounding box
  const gradient = new fabric.Gradient({
    type: 'radial',
    coords: {
      x1: 0,   // inner-circle center
      y1: 0,
      r1: 0,   // start radius (0 = center point)
      x2: 0,   // outer-circle center (same as x1/y1 for concentric)
      y2: 0,
      r2: 50   // end radius equals the circle’s radius
    },
    colorStops: [
      { offset: 0,   color: '#006dff' },  // deep blue center
      { offset: 0.6, color: '#28c4ff' },  // cyan mid-tone
      { offset: 1,   color: '#a0eaff' }   // very light edge
    ]
  });
      ...
      try {
        fabric.Image.fromURL(
          ...
          (img) => {
            ...
            img.applyFilters(); // Important to apply filters
            img.set({
              left: 350,
              top: 50,
              scaleX: 0.5,
